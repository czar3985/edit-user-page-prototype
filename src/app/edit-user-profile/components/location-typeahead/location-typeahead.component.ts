...
export class LocationTypeaheadComponent {
    ...
    onSelected(event: MatAutocompleteSelectedEvent): void {
        this.optionSelected.emit(event.option.value as LocationSearchItem);
    }

    getDescription(option: LocationSearchItem): string {
        if (option.parentLocationId && option.parentLocationName) return `Parent Location: ${option.parentLocationName}`;
        if (option.sublocations?.length) return `Sub Locations: ${option.sublocations.map((s) => s.name).join(', ')}`;
        return '';
    }

    displayLocation(option?: LocationSearchItem | string | null): string {
